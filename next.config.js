    ...
    if (dev && !isServer) {
      config.plugins.push(
        new CopyWebpackPlugin({
          patterns: [
            {
              from: path.join(__dirname, 'public/service-worker3.js'),
              to: path.join(__dirname, '.next', 'service-worker3.js')
            }
          ]
        })
      );
      ...
      //     swSrc: path.join(__dirname, 'public/service-worker3.js'),
      //     swDest: path.join(__dirname, '.next', 'service-worker3.js'),
