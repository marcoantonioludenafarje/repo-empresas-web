...
const EditDistributionDeliveryModal = ({
  ...
  useEffect(() => {
    ...
    if (selectedDeliveryState) {
      ...
      console.log(
        'Esto es lo que se ecibe en el componenete edit',
        selectedDeliveryState,
      );
      ...
      let productsToSet =
        selectedDeliveryState.products ||
  ...
  const saveCarrier = (carrier) => {
    setTemporaryDelivery({
      ...temporaryDelivery,
      carrierDenomination: carrier.denominationCarrier
        ? carrier.denominationCarrier
        : carrier.carrierDenomination,
      carrierDocumentType: carrier.typeDocumentCarrier
        ? carrier.typeDocumentCarrier
        : carrier.carrierDocumentType,
      carrierDocumentNumber: carrier.numberDocumentCarrier
        ? carrier.numberDocumentCarrier
        : carrier.carrierDocumentNumber,
    });
  ...
  const getNewProduct = (product) => {
    ...
    const productExists = productsList.some(
      (obj) =>
        obj.rowId === product.rowId || obj.productId === product.productId,
    );
  ...
  const addInputToProducts = (preCount, rowIdSelected, newProducts) => {
    ...
    let valToMultiply =
      isNaN(preCount) || preCount === '' || preCount === 0 ? 1 : preCount;
    let internCounter = counter;
    newProducts.forEach((prod) => {
      let existingProduct = productsList.find(
        (p) => p.product === prod.productId.replace(/^(0+)/g, '').split('-')[0],
      );
      if (existingProduct) {
        existingProduct.count += fixDecimals(
          valToMultiply * Number(prod.quantity),
        );
        ...
        <Grid container spacing={2} sx={{width: 'auto'}}>
          ...
          <Grid item xs={12} sm={6}>
            <Autocomplete
              ...
              renderInput={(params) => (
                <TextField
                  {...params}
                  label={<IntlMessages id='ubigeo.startingDistrictUbigeo' />}
                />
              )}
            />
          ...
          <Grid item xs={12} sm={6}>
            <Autocomplete
              ...
              renderInput={(params) => (
                <TextField
                  {...params}
                  label={<IntlMessages id='ubigeo.arrivalDistrictUbigeo' />}
                />
              )}
            />
          ...
          >
            <FormControlLabel
              ...
              label={<IntlMessages id='message.generate.referralGuide' />}
            />
          </Grid>
          <Grid item xs={12}>
            <Typography sx={{m: 2}}>
...
export default EditDistributionDeliveryModal;

EditDistributionDeliveryModal.defaultProps = {};

EditDistributionDeliveryModal.propTypes = {
