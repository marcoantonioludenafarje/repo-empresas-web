...
const Distribution = (props) => {
  ...
  function formatData(deliveries, originalPoints, arrivalPoints, drivers, carriers, products) {
    return deliveries.map(item => {
      ...
      const matchDriver = drivers.find(d => d["NRO IDENTIFICADOR"] == driver.split("-")[1].trim());
      ...
      productsSelected.forEach((product) => {
        let productInfo = products.find((item2) => item2["DESCRIPCION"] === product.product);
        if (productInfo["DOSIFICACION"]) {
          productInfo["DOSIFICACION"].split("|").forEach((inputProduct) => {
            let productInfo2 = products.find((item3) => item3["DESCRIPCION"] === inputProduct.split("-")[0].trim());
            if (productInfo2) {
              let existingProduct = productsInfo.find(p => p.product === productInfo2["CODIGO"]);
              if (existingProduct) {
                existingProduct.count += parseInt(inputProduct.split("-")[1].trim()) * product.quantity;
              } else {
                productsInfo.push({
                  productId: `${productInfo2["CODIGO"].padStart(32, "0")}-${userDataRes.merchantSelected.merchantId}`,
                  product: productInfo2["CODIGO"],
                  description: productInfo2["DESCRIPCION"],
                  unitMeasure: productInfo2["UNIDAD DE MEDIDA"],
                  typeProduct: productInfo["TIPO PRODUCTO"] === "Producto intermedio"
                    ? "intermediateProduct"
                    : productInfo["TIPO PRODUCTO"] === "Producto final"
                      ...
                      : productInfo["TIPO PRODUCTO"] === "Insumo"
                        ? "rawMaterial"
                        : null,
                  count: parseInt(inputProduct.split("-")[1].trim()) * product.quantity,
                  weight: productInfo2["PESO (Kg)"]
                });

              }
              totalWeight += parseInt(inputProduct.split("-")[1].trim()) * product.quantity * Number(productInfo2["PESO (Kg)"]);

            }
          });
        } else {
          if (productInfo) {
            let existingProduct = productsInfo.find(p => p.product === productInfo["CODIGO"]);
            if (existingProduct) {
              existingProduct.count += product.quantity;
            } else {
              productsInfo.push({
                productId: `${productInfo["CODIGO"].padStart(32, "0")}-${userDataRes.merchantSelected.merchantId}`,
      ...
      return {
        ...
        observationDelivery: "",
        generateReferralGuide: item["DESEA GENERAR GUIA REMISION?"] == "SI" ? true : false,
