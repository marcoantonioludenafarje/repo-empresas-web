...
const TagCard = ({
  ...
  const [selectedTag, setSelectedTag] = React.useState('noCategories');
  const counts = {};
  ...
  useEffect(() => {
    ...
    if (initialValues && initialValues.empty != true) {
      ...
      changeValue('observationDelivery', initialValues.tagName);
      setSelectedTag(initialValues);
      let newProds = [];
  ...
  const submitSetSubmitting = (data, setSubmitting) => {
    ...
    newValuesData(order, selectedTag);
    setSubmitting(false);
  };
  ...
  const handleChange = (event) => {
    if (event.target.name.includes('observationDelivery')) {
      let updateTag = selectedTag;
      updateTag.tagName = event.target.value;
